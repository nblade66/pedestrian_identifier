...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(-1, 40*6*14)                 # flattening of a tensor object (see Torch.tensor),
...
if __name__ == '__main__':
    ...
    if args.single_input == None:
        ...
        if create_plot == True:
            ...
            plt.title('Test1: Training Accuracies (Red) and Loss (Blue)')
            plt.ylabel('accuracies/losses')
            plt.xlabel('epoch')
            plt.savefig('test1_training_plot.png')

            # plot the test accuracies and losses
            plt.clf()
            plt.plot(test_x, test_y_accuracies, 'r', test_x, test_y_losses, 'b')
            plt.title('Test1: Test Accuracies (Red) and Loss (Blue)')
            plt.ylabel('accuracies/losses')
            plt.xlabel('epoch')
            plt.savefig('test1_testing_plot.png')
